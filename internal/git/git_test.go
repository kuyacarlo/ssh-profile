...
func mockRemoteExecCommand(command string, args ...string) *exec.Cmd {
	cs := []string{"-test.run=TestRemoteHelperProcess", "--", command}
	cs = append(cs, args...)
...
func TestRemoteHelperProcess(t *testing.T) {
	...
	switch len(args) {
	case 4: // git remote get-url upstream
		fmt.Fprint(os.Stdout, "github.com:github/github")
		os.Exit(0)
	case 5: // git remote set-url github.com:github/github
		os.Exit(0)
	}
...
func TestGetRemote(t *testing.T) {
	...
	g := &Git{exec: mockRemoteExecCommand}
	value, err := g.GetRemote("upstream")

	is.NoErr(err)
	is.Equal(value, "github.com:github/github")
}
...
func TestSetRemote(t *testing.T) {
	...
	g := &Git{exec: mockRemoteExecCommand}
	err := g.SetRemote("upstream", "github.com:github/github")
